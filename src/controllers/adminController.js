...
exports.modifyUserDetails = asyncHandler(async (req, res) => {
  ...
  exports.modifyAdminDetails = asyncHandler(async (req, res) => {
    try {
      ...
      const { email, phone, name, password, role} = req.body;
    const profileImage = req.file ? req.file.filename : updatedAdmin.profileImage;
  ...
  exports.deleteAdmin = asyncHandler(async (req, res) => {
    try {
      ...
      res.status(200).send({ message: "Admin is deleted successfully" });
    } catch (error) {
      console.log(error);
      res.status(500).send({ message: error.message });
    }
